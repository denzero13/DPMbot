...
from function import regex_kod, category
...
class Gift:
    ...
    def data_formation(self):
        with open('documents/import.txt', 'tw', encoding='utf-8') as file:
            ...
            iteration = 1
            for cteg in self.category:
                # Category
                file.write(f'// question: 0  name: Switch category to $cat1$/top/{self.main_category}/{cteg}\n')
                file.write(f'$CATEGORY: /top/{self.main_category}/{cteg}'+'\n'*3)

                for i in range(2, self.number_row):

                    variant = {'A' : '~', 'B' : '~', 'C' : '~', 'D' : '~'}  # Answer value
                    ...
                    if cteg == self.sheet.cell(row=i, column=8).value:
                        # Question name
                        file.write(f'// question: {iteration} name: '+str(self.sheet.cell(row=i, column=1).value)+'\n' )
                        # Kod in tag
                        file.write(f'// [tag:{cteg}]'+'\n' )
                        # Question title
                        file.write(f'::{cteg}\t'+str(self.sheet.cell(row=i, column=1).value))
                        file.write(f'\:::[html]{cteg}\t'+str(self.sheet.cell(row=i, column=1).value)+'\:{'+'\n')
                        # Answers
